...
## Status: V0.1 through V0.7 complete
...
|---|---|---|
| Frontend | static HTML + CSS + JS, all three sections `fetch()`ing the backend, dataset previews on click | V0.1 + V0.2 complete; connected to real, persisted backend data as of V0.5; real per-dataset preview UI as of V0.7. |
| Backend | FastAPI (`lab/main.py`), raw `http.server` kept as reference (`lab/server.py`) | V0.3-V0.7 complete — four routes (incl. file-backed dataset preview), decorator routing, auto JSON, CORS configured, real DB-backed persistence, logging + error handling across all routes. Next: V0.8 (Pandas + EDA) — see [[08-Next-Step]] |
