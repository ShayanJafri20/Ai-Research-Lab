...
**V0.1 through the core of V0.4 all complete.** HTML/CSS/JS foundation solid. V0.3 (2026-08-24) added HTTP fundamentals, a Python venv, and a raw `http.server` backend (`lab/server.py`, kept as a reference). V0.4 (2026-08-25) added FastAPI (`lab/main.py`), decorator-based routing, auto JSON, CORS middleware, and rewired the frontend's Models section to `fetch()` real data from it — the project's first working frontend-to-backend round trip. Next: extend `/datasets`+`/experiments` the same way, or move to **V0.5** (PostgreSQL) — data is still hardcoded in `main.py`, not persisted. User to decide which first. Live detail in [[08-Next-Step]].
