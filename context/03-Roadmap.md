...
- **V1.0-V1.3 — NLP/tokenization/RNN→Transformer**: expanded into its own full document, [[10-NLP-Research-Track]], on 2026-08-30 per explicit user request for the full-depth treatment (papers per stage, real math/dimensions, weight-training mechanics, "why this file exists" applied to ML code). Covers, in order: neural network foundations (sigmoid/tanh, backprop, an XOR MLP sandbox — before tokenization or the RNN itself), tokenization/UNK/masking + RNN (V1.0, Elman 1990), LSTM/GRU (V1.1, Hochreiter & Schmidhuber 1997), Attention (V1.2, Bahdanau et al. 2014), Transformer + BPE (V1.3, Vaswani et al. 2017). See that file for the full plan — this line just points to it, no need to duplicate.
