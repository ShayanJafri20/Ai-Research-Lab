...
- **Pre-V1.0 — neural network foundations, expanded 2026-08-30 per explicit user request for full depth (papers, math, dimensions, "why this file exists"):** before tokenization or the RNN itself, a hand-built MLP (pure Python/NumPy, no framework) on the classic XOR problem, as a Sandbox exercise (see [[07-Sandbox-Log]]) — covers sigmoid/tanh activation functions, how layers connect (weighted sums + activation), forward pass, backpropagation/gradient descent by hand, and *why* more than one layer is needed at all (XOR isn't linearly separable). This is the actual foundation an RNN builds on ("an MLP plus a loop over time, sharing weights").
  - **V1.0 — tokenization, applied to real Gutenberg text:** word-level tokenization, vocabulary building, UNK tokens (handling words not in the vocabulary), padding/masking (why variable-length sequences need it, once batching multiple real texts together). Then the RNN itself: hidden state, recurrence, why weights are shared across time steps. Reference: **Elman (1990), "Finding Structure in Time"** (the original simple RNN paper) + Andrej Karpathy's "The Unreasonable Effectiveness of Recurrent Neural Networks" (accessible companion, not a formal paper). PyTorch + `DataLoader` introduced here, once hand-rolled backprop from the MLP step is genuinely felt as painful — not before.
  - **V1.1 — LSTM/GRU:** motivated by RNN's specific failure (forgetting long-range context). Reference: **Hochreiter & Schmidhuber (1997), "Long Short-Term Memory."**
  - **V1.2 — Attention:** motivated by LSTM's remaining sequential bottleneck. Reference: **Bahdanau et al. (2014), "Neural Machine Translation by Jointly Learning to Align and Translate."**
  - **V1.3 — Transformer + BPE:** Reference: **Vaswani et al. (2017), "Attention Is All You Need."** Also where "why does this config file exist" gets answered concretely for real Transformer training code (hard rule 9 applied to ML project structure), and where separate training/validation code/data splits become concrete practice, ahead of full formalization at V1.4 (Experiment tracking).
