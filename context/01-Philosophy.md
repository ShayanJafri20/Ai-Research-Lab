...
[[03-Roadmap]]'s Notes section has referenced this section since the vault was first built, but it was never actually written until a direct question exposed the gap. These three don't get dedicated roadmap versions — they get folded into the versions that already create a real reason for them, per hard rule 3.
...
- **Power BI** (or an open equivalent): a genuine "one layer above" exercise — once PostgreSQL (V0.5+) holds real data, connecting an external BI tool and building a business-style dashboard shows how non-technical tools consume a database differently than the app's own code does. Likely real trigger: after V0.9 (ETL) has real, clean data worth visualizing at that level, or alongside V1.4 (Experiment tracking) for comparing runs visually outside the app itself. No fixed version — introduce it the first time "I want to show this data to someone without them touching code" becomes a real want. DAX (Power BI's formula language for calculated measures) rides along with this, not a separate thing to schedule.
