...
**The predicted seed-data gap from the note above was real** — had to hand-retype the `INSERT` statements from the setup notes rather than run anything scripted, exactly as flagged. Now that it's been felt twice (predicted, then actually painful), a small `lab/seed.sql` is worth writing next time this comes up, so a third machine wouldn't need manual re-typing at all.
